...
async function generateRandomGraph() {
  ...
  try {
    ...
    const response = await fetch(`${API_MINCYCLE_URL}/generate`, {
      method: "POST",
      headers: {
...
async function analyzeAllNodes(formattedGraph) {
  ...
  try {
    ...
    const response = await fetch(`${API_MINCYCLE_URL}/analyze`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
async function fetchModels() {
  ...
  try {
    const response = await fetch(`${API_MINCYCLE_URL}/models`);
    const data = await response.json();
