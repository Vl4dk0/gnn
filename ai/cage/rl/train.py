...
if __name__ == "__main__":
    ...
    train_ppo(
        ...
        # Bootstrap value for the last step
        with torch.no_grad():
            next_value = agent(obs)[1].item()

        # Compute GAE
        values = value_buffer + [next_value]
        advantages = compute_gae(
            reward_buffer, values, next_value, done_buffer, gamma, gae_lambda
        )
        advantages = advantages.to(device)
        returns = advantages + torch.tensor(value_buffer, device=device)
        ...
        # Logging
        avg_rew = sum(episode_rewards) / len(episode_rewards) if episode_rewards else 0
        avg_len = sum(episode_lens) / len(episode_lens) if episode_lens else 0
        fps = int(global_step / (time.time() - start_time))
        print(
            f"Step {global_step} | Avg Reward: {avg_rew:.2f} | Avg Len: {avg_len:.1f} | FPS: {fps}"
        )
    ...
    save_path = save_model(
        ...
        task="cage",
        model_id=model_id,
        metrics={"avg_reward": round(best_avg_reward, 2), "fps": fps},
        training_info=training_info,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name", "-n", type=str, required=True, help="Model version name"
    )
    ...
    parser.add_argument("--lr", type=float, default=3e-4, help="Learning rate")
    parser.add_argument(
        "--update-interval", type=int, default=2048, help="PPO update interval"
    )
    parser.add_argument(
        "--print-every",
        type=int,
    ...
    parser.add_argument(
        "--no-random", action="store_true", help="Disable curriculum randomization"
    )
    parser.add_argument(
        "--force", "-f", action="store_true", help="Overwrite existing model"
    )
