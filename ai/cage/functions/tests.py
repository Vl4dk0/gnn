...
from backend.utils.graph_utils import is_valid_cage, compute_girth, moore_bound


class _GeneratorProto(Protocol):
    """Protocol for cage generator instances."""
...
class _GeneratorFactory(Protocol):
    """Protocol for cage generator classes (callable as constructor)."""

    __name__: str

    def __call__(self, k: int, g: int) -> _GeneratorProto: ...
...
class CageTest:
    ...
    def __init__(self, k: int, g: int, name: str | None = None):
        self.k = k
        self.g = g
...
def run_single_test(
    generator_class: _GeneratorFactory,
    test: CageTest,
    max_steps: int = 10000,
    verbose: bool = False,
) -> GeneratorTestResult:
...
def run_test_suite(
    ...
    """
    results: list[GeneratorTestResult] = []
...
def print_summary(results: list[GeneratorTestResult]) -> None:
    ...
    # Group by generator
    by_generator: dict[str, list[GeneratorTestResult]] = {}
    for r in results:
    ...
    for gen_name, gen_results in by_generator.items():
        ...
        if total_steps:
            ...
            print(
                f"  Steps (successful): median={median_steps:.0f}, mean={mean_steps:.0f}"
            )


def compare_generators(
    generator_classes: list[_GeneratorFactory],
    tests: list[CageTest],
    max_steps: int = 10000,
    verbose: bool = False,
) -> None:
    ...
    Args:
        ...
        verbose: Print detailed progress
    """
    all_results: list[GeneratorTestResult] = []
...
def test_small_cages(
    generator_class: _GeneratorFactory, max_steps: int = 10000, verbose: bool = False
) -> list[GeneratorTestResult]:
...
if __name__ == "__main__":
    ...
    _ = parser.add_argument(
        "--generator",
        choices=["randomwalk", "bruteforce", "astar", "all"],
        default="randomwalk",
        help="Which generator to test (default: randomwalk)",
    )
    _ = parser.add_argument(
        "--steps", type=int, default=10000, help="Max steps per test (default: 10000)"
    )
    _ = parser.add_argument(
    ...
    generator_arg = cast(str, args.generator)
    suite_arg = cast(str, args.suite)
    steps_arg = cast(int, args.steps)
    verbose_arg = cast(bool, args.verbose)

    # Select generators
    generators: list[_GeneratorFactory] = []
    if generator_arg in ["randomwalk", "all"]:
        generators.append(RandomWalkGenerator)
    if generator_arg in ["bruteforce", "all"]:
        generators.append(BruteforceGenerator)
    if generator_arg in ["astar", "all"]:
        generators.append(AStarGenerator)

    # Select test suite
    if suite_arg == "small":
        tests = SMALL_CAGES
    elif suite_arg == "medium":
        tests = MEDIUM_CAGES
    elif suite_arg == "large":
        tests = LARGE_CAGES
    else:
    ...
    # Run tests
    if len(generators) == 1:
        results = run_test_suite(generators[0], tests, steps_arg, verbose_arg)
        print_summary(results)
    else:
        compare_generators(generators, tests, steps_arg, verbose_arg)
