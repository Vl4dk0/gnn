...
def generate_random_graph_data(num_nodes_range=(5, 12), p_range=(0.15, 0.6)):
    ...
    x = torch.cat([node_idx_feature, random_feature, clustering_feature], dim=1)  # Shape: [num_nodes, 4]
...
def save_model_info(metrics, epoch, hidden_dim, lr, path='backend/models/model_info.json'):
    """
...
def train_gnn(num_epochs=5000, hidden_dim=64, lr=0.005, print_every=100, eval_every=20, graphs_per_epoch=10):
    """
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == 1:
            ...
            if is_better:
                ...
                print(f"  ✓ New best model! Accuracy: {best_accuracy:.2f}%, MAE: {best_mae:.4f}")
...
def load_model(path='backend/models/trained_gnn.pt', hidden_dim=None, input_dim=4):
    """
    ...
    if os.path.exists(path):
        ...
        model.eval()
        print(f"Model loaded from {path} (hidden_dim={hidden_dim}, input_dim={input_dim})")
    else:
