...
def load_gnn_model():
    ...
    try:
        ...
        # 4 features: idx + random(2) + clustering
        _model = DegreeGNN(hidden_dim=hidden_dim, input_dim=4)
        _model.load_state_dict(torch.load(model_path, weights_only=True))
        _model.eval()
        print(
            f"GNN model loaded from {model_path} (hidden_dim={hidden_dim}, input_dim=4)"
        )
...
def predict_degree_with_gnn(G: nx.Graph, vertex: int) -> float:
    ...
    try:
        ...
        num_nodes = len(G.nodes())  # type: ignore
