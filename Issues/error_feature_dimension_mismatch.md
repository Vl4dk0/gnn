...
The thesis states: "The experiments use either a constant one-dimensional feature or a four-dimensional feature vector". This implies models are trained and evaluated with both 1D and 4D input features. However, in both the `degree` and `min_cycle` API endpoints (`graph_service.py`), the input feature tensor `x` is hardcoded to always concatenate into a 4-dimensional tensor (`x = torch.cat([node_idx_feature, random_feature, clustering_feature], dim=1)`).
...
To resolve this, the code should check the loaded model's `input_dim` (e.g., `getattr(model, "input_dim", 4)`). If `model.input_dim == 1`, it should construct `x` as a constant one-dimensional tensor (`torch.ones(num_nodes, 1)`), exactly matching the logic in `generate_random_graph_data` inside `train.py`. The 4-dimensional features should only be concatenated if `model.input_dim == 4`.
